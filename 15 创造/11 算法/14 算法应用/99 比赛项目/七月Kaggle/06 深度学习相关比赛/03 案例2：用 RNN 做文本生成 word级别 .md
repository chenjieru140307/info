...
我们需要把我们的 raw text变成可以用来训练的 x,y:

x 是前置单词们， y 是后一个单词。我们把 10 个单词的词向量作为 x。
...
1. 我们已经有了一个 input 的数字表达（w2v），我们要把它变成 LSTM 需要的数组格式： [样本数，时间步伐，特征]
2. 第二，对于 output，我们直接用 128 维的输出
...
<span style="color:red;">为什么 lstm 是 256 个神经元？而且 dropout_W 和 dropout_U 是什么？与 Dropout 层有什么区别？</span>
