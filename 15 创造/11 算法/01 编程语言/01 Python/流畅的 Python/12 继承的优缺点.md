...
要的方法，在控制台中输入 dir（tkinter.Button），你会发现列出了 214个属性，9此
...
（[https://en+wikipedia+org/wiki/Template_method_patter^](https://en.wikipedia.org/wiki/Template_method_pattern)）。之所以说是动态的，是因为 View 类不强制子类实现所有处理
