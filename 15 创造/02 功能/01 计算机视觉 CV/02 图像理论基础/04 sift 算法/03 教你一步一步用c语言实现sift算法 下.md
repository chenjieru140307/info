...
CvMat *ScaleInitImage(CvMat * im)
{
	...
	{
		tempMat = doubleSizeImage2(imMat);//对扩大两倍的图像进行二次采样，采样率为 0.5，采用线性插值
#define TEMPMAT(ROW,COL) ((float *)(tempMat->data.fl + tempMat->step/sizeof(float) * (ROW)))[(COL)]
		...
		BlurImage(dst, tempMat, sigma);       //得到金字塔的最底层-放大 2 倍的图像
		cvReleaseMat( &dst );
		return tempMat;
	}
	else
...
SIFT第二步，建立 Gaussian 金字塔，给定金字塔第一阶第一层图像后，计算高斯金字塔其他尺度图像，
...
ImageOctaves* BuildGaussianOctaves(CvMat * image)
{
	...
	{
		//首先建立金字塔每一阶梯的最底层，其中 0 阶梯的最底层已经建立好
		...
		for ( j =  1; j < SCALESPEROCTAVE + 3; j++)
		{
			...
			temp = cvCreateMat(tempMat->rows, tempMat->cols, CV_32FC1);//用于存储 DOG 层
...
SIFT算法第三步，特征点位置检测，最后确定特征点的位置检测 DOG 金字塔中的局部最大值，找到之后，还要经过两个检验才能确认为特征点：一是它必须有明显的差异，二是他不应该是边缘点，（也就是说，在极值点处的主曲率比应该小于某一个阈值）。

```c
//SIFT算法第三步，特征点位置检测，
int DetectKeypoint(int numoctaves, ImageOctaves *GaussianPyr)
{
	//计算用于 DOG 极值点检测的主曲率比的阈值
...
#define ImLevels(OCTAVE,LEVEL,ROW,COL) ((float *)(DOGoctaves[(OCTAVE)].Octave[(LEVEL)].Level->data.fl + DOGoctaves[(OCTAVE)].Octave[(LEVEL)].Level->step/sizeof(float) *(ROW)))[(COL)]
	...
	{
		for(int j=1;j<SCALESPEROCTAVE+1;j++)//取中间的 scaleperoctave 个层
		{
					...
					if ( fabs(ImLevels(i,j,m,n))>= CONTRAST_THRESHOLD )
					{
						...
						if ( ImLevels(i,j,m,n)!=0.0 )  //1、首先是非零
						{
							...
							if(( (inf_val <= ImLevels(i,j-1,m-1,n-1))&&
								...
								(inf_val <= ImLevels(i,j-1,m-1,n+1))&&
								(inf_val <= ImLevels(i,j-1,m  ,n+1))&&
								(inf_val <= ImLevels(i,j-1,m+1,n+1))&&    //底层的小尺度 9
								...
								(inf_val <= ImLevels(i,j,m-1,n+1))&&
								(inf_val <= ImLevels(i,j,m  ,n+1))&&
								(inf_val <= ImLevels(i,j,m+1,n+1))&&     //当前层 8
								...
								(inf_val <= ImLevels(i,j+1,m-1,n+1))&&
								(inf_val <= ImLevels(i,j+1,m  ,n+1))&&
								(inf_val <= ImLevels(i,j+1,m+1,n+1))     //下一层大尺度 9
								) ||
								( (inf_val >= ImLevels(i,j-1,m-1,n-1))&&
								(inf_val >= ImLevels(i,j-1,m  ,n-1))&&
								(inf_val >= ImLevels(i,j-1,m+1,n-1))&&
							...
							{
								//此处可存储
								//然后必须具有明显的显著性，即必须大于 CONTRAST_THRESHOLD=0.02
								if ( fabs(ImLevels(i,j,m,n))>= CONTRAST_THRESHOLD )
								{
									//最后显著处的特征点必须具有足够的曲率比，CURVATURE_THRESHOLD=10.0，首先计算 Hessian 矩阵
...
{
	double a = ((left + right) - 2.0 * middle) / 2.0;   //抛物线捏合系数 a
...
SIFT算法第五步：抽取各个特征点处的特征描述字，确定特征点的描述字。描述字是 Patch 网格内梯度方向的描述，旋转网格到主方向，插值得到网格处梯度值。

一个特征点可以用 2*2*8=32维的向量，也可以用 4*4*8=128维的向量更精确的进行描述。
...
![](http://images.iterate.site/blog/image/180708/B22lDg0DCK.png?imageslim){ width=55% }

![](http://images.iterate.site/blog/image/180708/9kDeD1HDb0.png?imageslim){ width=55% }


![](http://images.iterate.site/blog/image/180708/JHhIBj972c.png?imageslim){ width=55% }

![](http://images.iterate.site/blog/image/180708/2gf4de0kjI.png?imageslim){ width=55% }

![](http://images.iterate.site/blog/image/180708/ccH59j9hcA.png?imageslim){ width=55% }
