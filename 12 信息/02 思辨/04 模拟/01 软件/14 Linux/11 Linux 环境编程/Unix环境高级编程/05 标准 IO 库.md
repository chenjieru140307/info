...
使用 tmpnam 和 tempnam 至少有一个缺点：在返回唯一的路径名和用该名字创建文件之间 存在一个时间窗口，在这个时间窗口中，另一进程可以用相同的名字创建文件，因此应该使用
...
这个例子给出了冲洗内存流和追加写 null 字节的策略。写入内存流以及推进流的内容大小（相 对缓冲区大小而言，该大小是固定的）这个概念时，null字节会自动追加写。流内容大小是由写 入多少来确定的。
