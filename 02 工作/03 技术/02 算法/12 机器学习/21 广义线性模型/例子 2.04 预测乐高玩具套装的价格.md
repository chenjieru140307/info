---
title: 例子 2.04 预测乐高玩具套装的价格
toc: true
date: 2018-08-03 12:12:13
---
# 可以补充进来的

- <span style="color:red;">还是有很多问题的，在下面有写</span>
- <span style="color:red;">要把 cross_valiadation 拆分出来，太臃肿了，要只实现交叉采样的功能</span>
...
书上说用 Google Shopping 的 API 收集数据，从返回的 JSON 数据中抽取价格。
...
# 在 python3 中将 urllib2 和 urllib3 合并为一个标准库 urllib，其中的 urllib2.urlopen更改为 urllib.request.urlopen
...
def search_for_set_with_google_api(datas, labels, set_num, yr, num_pce, orig_prc):
    ...
    info_dict = json.loads(page.read())  # 转换为 json 格式
...
def cross_validation(data_arr, label_arr, num_val=10):
    ...
    for i in range(num_val):
        ...
        for k in range(30):  # loop over all of the ridge estimates
            # weights_mat[k, :] 是 np.array格式的，如果不转换成 np.mat 则无法通过.T 转化成列向量
...
if __name__ == "__main__":
    datas = []
    labels = []
    # get_data_set(datas,labels) #怎么正确使用 google 现在的 api？
...
注意读取 html 文件的时候的 encoding='utf-8' 。**是不是用 utf-8就一定对？**
...
**而且，不仅 google 的，baidu的，携程的等等都要总结下。**


## array 是不能.T的，一定要线转化成 np.mat


**要总结到 numpy的 array 与 mat 的文章里面。**
...
## 这个输出的 rss_error好像很大呀，这个是正常结果吗？


**未确认**


## 对于 ridge_regression 的理解还是不够，理论结合这里的代码再看下
