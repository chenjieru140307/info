...
- [文档](https://itchat.readthedocs.io/zh/latest/tutorial/tutorial0/#_1)
...
- 把原本的脚本里网络构成和训练的部分甩到了一个函数nn_train里
- 在有print的地方都加了个itchat.send来输出日志
- 此外加了个带锁的状态量running用来做运行开关。
- chat_trigger 作用是，如果收到微信消息，内容为『开始』，那就跑训练的函数（当然，为了防止阻塞，放在了另一个线程里）
- 在 main 中使用itchat登录微信并且启动itchat的服务，这样就实现了基本的控制。