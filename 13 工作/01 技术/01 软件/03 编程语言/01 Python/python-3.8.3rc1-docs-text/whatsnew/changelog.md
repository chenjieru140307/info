...
* bpo-38117: 将绑定的OpenSSL更新到1.1.1d
...
* bpo-36686: Improve documentation of the stdin, stdout, and stderr
  arguments of of the "asyncio.subprocess_exec" function to specify
  which values are supported. Also mention that decoding as text is
  not supported.
...
* bpo-35926: 在Windows安装程序中将OpenSSL更新为1.1.1b
...
* bpo-36742: 于函数 urlsplit() 中修复了对于预规格化字符的不当处理过程
  。
...
* bpo-36504: 在 _ctypes.c's "PyCArrayType_new()" 中修复了“有符号整数”
  类型的溢出。

* bpo-20844: 修复了含有经编码的 cookie 和换行符的脚本位于 Windows 平台
  可能运行失败的问题。

* bpo-24214: 修复了UTF-8增量解码器对surrogatepass的错误回调方法的支持
...
* bpo-36459: 修复了由于tokenizer.c中的 "tok_nextc()" 方法可能导致的两
  次 "PyMem_FREE()" 调用。

* bpo-36433: 修复了位于 classmethoddescr_call 的类型错误信息

* bpo-36430: 修复了函数 "itertools.count()" 中的潜在引用对象泄漏。
...
* bpo-31292: 为包含 "include" 指令的文件修复 "setup.py check
  --restructuredtext"。
...
* bpo-36236: 在 Python 初始化时，如果当前目录被移除，则它将不再会被添
  加到 "sys.path" 中。
...
* bpo-36262: 修复了在 "float(str)", "complex(str)", "pickle.load()",
  "marshal.load()" 等所用的函数 "_Py_dg_strtod()" 中进行从字符串到浮点
  数的转换时偶有发生的一个内存泄漏问题。
...
* bpo-36176: 修复 IDLE 自动补全和 calltip 悬浮窗的颜色. 避免与 Linux
  暗色主题冲突 (略微加深了calltip 的背景颜色).
...
* bpo-34405: 将Windows内部版本更新到OpenSSL 1.1.0i

* bpo-33871: 修复了使用 "os.sendfile()" 在macOS上发送部分文件的错误。
...
* bpo-35623: 修复了在排序超长列表时崩溃的问题。 补丁作者 Stephan Hohe
  。

* bpo-35214: 添加了 clang Memory Sanitizer 构建工具以绕过来自 posix,
  socket, time, test_io 和 test_faulthandler 的误报问题。

* bpo-35560: 修复了在调试版中使用 "n" 格式进行零留空和小宽度的浮点格式
  化时 "format()" 内的一个断言错误。 发布版未受影响。 补丁作者
  Karthikeyan Singaravelan。
...
* bpo-35504: 修复了在删除特定属性时的段错误和 "SystemError" 问题。 补
  丁作者 Zackery Spytz。
...
* bpo-34854: 修复问题——当编译包含一个没有默认值的关键字参数的 lambda
  的字符串注解时可导致崩溃。
...
* bpo-34170: -X dev: 即使启用了开发模式，现在也可以使用PYTHONMALLOC重
  载内存分配器。
...
* bpo-34126: 修复在分析某些未绑定的方法的无效调用时崩溃的问题。由
  Jeroen Demeyer修复。

* bpo-24618: 修复了当创建代码对象时使用的变量名元组太小或参数太多导致
  读取无效内存的问题。

* bpo-34068: 在调用 "io.IOBase.close()" 时确保 "closed" 属性未设置有效
  的异常。 由Zackery Spytz和Serhiy Storchaka修复。

* bpo-34087: 修复由unicode转换为数值类型时缓冲区溢出的问题。

* bpo-34080: 修复了编译器在标记化过程中引发一些不常见错误时内存泄漏的
  问题。

* bpo-34066: 调用 "open()" 和进入  "with open()" 的 **with** 语句块内
  时禁止使用 Ctrl-C 中断。

* bpo-34042: 修复 dict.copy() 以维护正确的总引用计数（由
  sys.gettotalrefcount()报告）。

* bpo-33418: Fix potential memory leak in function object when it
  creates reference cycle.

* bpo-33985: 实现contextvars.ContextVar.name 属性。

* bpo-33956: Update vendored Expat library copy to version 2.2.5.

* bpo-24596: 执行 "PyErr_Print()" 之前，在 "PyRun_SimpleFileExFlags()"
  内减少模块对象的引用。 由Zackery Spytz修复。

* bpo-33451: 执行 "PyEval_EvalCode()" 前先关闭直接执行的pyc文件。
...
* bpo-33824: 修复"LC_ALL=C python3.7 -V"：在读取Python配置后编码发生更
  改时，正确重置命令行解析器。
...
* bpo-33363: 在异步函数外使用``async with`` 和 "async for"  时抛出
  SyntaxError异常。

* bpo-28055: Fix unaligned accesses in siphash24(). Patch by Rolf Eike
  Beer.

* bpo-33128: 修复导致PathFinder在sys.meta_path上出现两次的错误。由
  Pablo Galindo Salgado提供补丁。
...
* bpo-28503: 当库函数可用时 "crypt" 模块现在将在内部使用 "crypt_r()"
  而非 "crypt()"。
...
* bpo-35585: 加快构建按值枚举的速度，例如 http.HTTPStatus(200)。
...
* bpo-35513: "unittest.runner" 的 "TextTestRunner" 现在将使用
  "time.perf_counter()" 而非 "time.time()" 来计量一个测试的执行时间:
  "time.time()" 可以回退，而 "time.perf_counter()" 为单调递增。

* bpo-35502: 修复了构建树未完成的情况下
  "xml.etree.ElementTree.TreeBuilder" 中的引用泄漏（特别是在解析 XML
  期间引发错误的时候）。
...
* bpo-31446: 复制被传递给 CreateProcessW 的命令行，因为此函数能改变输
  入缓存的内容。

* bpo-35471: Python2.4 取消了对MacOS 9 的支持。Python3.7 中已弃用
  macpath 模块。该模块现在已移除。
...
* bpo-17185: 在 mock 上设置 "__signature__" 以便 "inspect" 能获得签名
  。 补丁作者为 Karthikeyan Singaravelan。
...
* bpo-35346: 从 "platform" 模块中删除 MacOS 9 和 Rhapsody 支持。
  Rhapsody 上一次发布是在2000年。MacOS 9 上一次发布是在2001年。
...
* bpo-34819: 使用一个单一时钟计算 "Executor.map()" 和 "as_completed()"
  中的超时，以防止当系统时钟被修改时因偏离导致的超时。
...
* bpo-34333: 修复在 "pathlib.PurePath.with_suffix()" 内使用
  %-formatting 格式化错误信息的错误。
...
* bpo-33604: Remove HMAC default to md5 marked for removal in 3.8
  (removal originally planned in 3.6, bump to 3.8 in gh-7062).
...
* bpo-33127: ssl模块现在使用LibreSSL 2.7.1编译。

* bpo-32505: 如果数据类的成员变量的类型为Field，但没有类型注解，则引发
  TypeError。

* bpo-33078: 修复在OSX平台因为依赖sem_getvalue而导致的失败。

* bpo-33116: 将'Field'添加到dataclasses.__all__。
...
* bpo-33061: 添加丢失的 "NoReturn" 到typing.py的 "__all__"
...
* bpo-33604: 更新HMAC md5默认报告DeprecationWarning，并将在3.8移除。
...
* bpo-26704: 添加了对一个实例方法进行双重修补的测试演示。 补丁作者为
  Anthony Sottile。
...
* bpo-33655: 忽略运行在BSD平台的ZFS上时test_posix_fallocate的失败。
...
* bpo-35401: 将Windows内部版本更新到OpenSSL 1.1.0j
...
* bpo-33184: 在Windows安装程序中将OpenSSL更新为1.1.0h.
...
* bpo-35401: 在MacOS安装程序中将OpenSSL更新为1.1.0j。
...
* bpo-34405: 在MacOS安装程序中将OpenSSL更新为1.1.0i 。
...
* bpo-33184: 在MacOS安装程序中将OpenSSL更新为1.1.0h。
...
* bpo-35769: 将 IDLE 的新文件名从 'Untitled' 更改为 'untitled'

* bpo-35660: Fix imports in idlelib.window.

* bpo-35641: 当函数没有文档字符串时正确地格式化 "calltip"。
...
* bpo-32604:  删除_xxsubinterpreters模块（用于测试）和相关的帮助程序。
  该模块最初最初是在3.7b1中添加的。
...
* bpo-32593: 放弃对 FreeBSD 9 及更早版本的支持。
...
* bpo-28791: 更新 Windows 构建以使用 SQLite 3.21.0.
...
* bpo-31392: 更新 macOS 安装程序以使用OpenSSL 1.0.2m
...
* bpo-29572: 更新Windows 构建和OS X安装程序以使用OpenSSL 1.0.2k。
...
* bpo-28208: 更新Windows 构建和OS X 安装程序以使用SQLite 3.14.2.

* bpo-28248: 更新Windows 构建和OS X安装程序以使用OpenSSL 1.0.2j.
...
* bpo-31392: 更新Windows 构建以使用OpenSSL 1.1.0f
...
* bpo-33184: 更新Windows 构建以使用OpenSSL 1.0.2o.


macOS
-----

* bpo-33184: 更新macOS 安装程序以使用OpenSSL 1.0.2o.
...
* bpo-27883: 在Windows上将sqlite更新到3.14.1.0
...
* bpo-26930: 更新Windows 构建以使用OpenSSL OpenSSL 1.0.2h.
...
* bpo-26930: 更新OS X 10.5+（仅32位）安装程序以构建并链接OpenSSL
  1.0.2h。
...
* bpo-26465: 更新Windows 构建OpenSSL 1.0.2g.
...
* bpo-26465: 更新 OS X 10.5+（仅32位）安装程序以构建并链接 OpenSSL
  1.0.2g.

* bpo-26268: 更新Windows 构建OpenSSL 1.0.2f.
...
* bpo-19450: 更新Windows和OS X安装程序以使用SQLite 3.8.11
...
* bpo-24603: 更新Windows 构建和OS X 10.5 安装程序以使用OpenSSL 1.0.2d.
...
* bpo-24432: 更新Windows 构建和OS X 10.5 安装程序以使用OpenSSL 1.0.2c.
...
* bpo-23686: 更新OS X 10.5 安装程序以使用OpenSSL 1.0.2a.
...
* bpo-22085: 在 Tkinter 中删除了对 TK 8.3 的支持。
...
* bpo-22592: 放弃对 Borland C 编译器构建Python的支持。distutils模块仍
  然支持它来构建扩展。

* bpo-22591: 放弃对 MS-DOS 的支持，尤其是 DJGPP 编译器（GCC的MS-DOS端
  口）。
...
* bpo-21275: 修复KFreeBSD上的套接字测试。

* bpo-21223: Pass test_site/test_startup_imports when some of the
  extensions are built as builtins.

* bpo-20635: 添加了针对Tk几何管理器的测试。

* 增加freeze包的测试用例。

* bpo-20743: 修复test_tcl中的引用泄漏。

* bpo-21097: 将test_namespace_pkgs 移至 test_importlib。
...
* bpo-21906: 让Tools/scripts/md5sum.py可以工作在Python 3。由Zachary
  Ware提供补丁。
...
* bpo-22644: OpenSSL的捆绑版本已更新到1.0.1j。
