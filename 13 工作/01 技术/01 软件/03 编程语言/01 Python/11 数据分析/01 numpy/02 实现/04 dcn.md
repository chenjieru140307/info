...
- [百度网盘](https://pan.baidu.com/s/1K0evKgFBZdHUDGfsQYxAyg) 提取码：j3h0


相关：

- [用纯NumPy码一个RNN、LSTM：这是最好的入门方式了](https://zhuanlan.zhihu.com/p/54868269)
...
def gradCheck(inputs, target, cprev, hprev, mprev, rprev):
    ...
    _, dWxh, dWhh, dWhy, dWhr, dWhv, dWhw, dWhe, dWrh, dWry, dbh, dby, _, _ = lossFun(inputs, targets, cprev, hprev,
                                                                                      mprev, rprev)
    print('GRAD CHECK\n')
...
def lossFun(inputs, targets, cprev, hprev, mprev, rprev, plot=False):
    ...
    for t in range(len(inputs)):
        ...
        memory[t] = memory[t - 1] * (
                1 - np.reshape(mem_erase_gate[t], (1, MW, B)) * np.reshape(mem_write_gate[t], (MN, 1, B)))
    ...
    for t in reversed(range(len(inputs))):
        ...
        dg[3 * N:4 * N, :] = dg[3 * N:4 * N, :] * (
                1 - gs[t][3 * N:4 * N, :] * gs[t][3 * N:4 * N, :])  # backprop through tanh
    ...
    return loss, dWxh, dWhh, dWhy, dWhr, dWhv, dWhw, dWhe, dWrh, dWry, dbh, dby, cs[len(inputs) - 1], hs[
        len(inputs) - 1]
...
if __name__ == "__main__":
    ...
    while t < T:
        ...
        loss, dWxh, dWhh, dWhy, dWhr, dWhv, dWhw, dWhe, dWrh, dWry, dbh, dby, cprev, hprev = lossFun(inputs, targets,
                                                                                                     cprev, hprev,
                                                                                                     mprev, rprev, plot)
...
def train(inputs, targets, cprev, hprev, mprev, rprev):
    ...
    for t in reversed(range(len(inputs))):
        ...
        dg[3 * HN:4 * HN, :] = dg[3 * HN:4 * HN, :] * (
                1 - gs[t][3 * HN:4 * HN, :] * gs[t][3 * HN:4 * HN, :])  # backprop through tanh
        dbh += np.expand_dims(np.sum(dg, axis=1), axis=1)
        dWxh += np.dot(dg, xs[t].T)
        dWhh += np.dot(dg, hs[t - 1].T)
        dWrh += np.dot(dg, rs[t - 1].T)
...
while t < T:
    ...
    # forward S characters through the net and fetch gradient
    loss, dWxh, dWhh, dWhy, dWhr, dWhv, dWhw, dWhe, dWrh, dWry, dbh, dby, cprev, hprev = train(inputs, targets, cprev,
                                                                                               hprev, mprev, rprev)
    smooth_loss = smooth_loss * 0.999 + np.mean(loss) / (np.log(2) * B) * 0.001
    if n % 10 == 0:
        tdelta = time.time() - last
        ...
        t = time.time() - start
        print(
            '%.3f s, iter %d, %.4f BPC, %.2f char/s' % (t, n, smooth_loss / S, (B * S * 10) / tdelta))  # print progress
