...
![](http://images.iterate.site/blog/image/180817/icHHK9A2FI.png?imageslim){ width=55% }

我们已经有了 CNN、RNN 工具

![](http://images.iterate.site/blog/image/180817/d4ALLAai8g.png?imageslim){ width=55% }

CNN 把一个图像变成了一个 tensor 的表示，会比变成一个 4096 的向量，他的 multi instance 的信息就包含进去了，就是说里面的每一个小柱子代表了图像中一个区域，这个是它的核心，这个事情是需要从机器学习的角度来看的，我们这个 attention 其实就是利用了这个表示，他把 tansor 拿出来了，这个每一个框框![](http://images.iterate.site/blog/image/180817/40hJf0Ij9f.png?imageslim){ width=55% }代表的是每一个小柱子，而这个框框是能够回溯到原图像中的具体的区域的，然后，我再针对于这些 instance 利用 RNN 我每次做一个 softmax，softmax 回归的时候，我需要 attention 的 instance 的 id。所以，一个是这些 instance 的一个线性叠加，一个是我的 caption 的 word，我就能每次预测出一个需要 attention  attend 到的一个 instance 的位置。<span style="color:red;">还是没理解，要弄明白，并且要弄清楚我对于这些知识的掌握还有哪些不足？</span>
...
![](http://images.iterate.site/blog/image/180817/5ach50chAi.png?imageslim){ width=55% }
...
![](http://images.iterate.site/blog/image/180817/chl77bmGfg.png?imageslim){ width=55% }
...
![](http://images.iterate.site/blog/image/180817/dKBD4hDdmk.png?imageslim){ width=55% }


4237 张鲸鱼的照片，有 427 个类别，对鲸鱼进行分类。

![](http://images.iterate.site/blog/image/180817/Ilci4AiJKb.png?imageslim){ width=55% }
...
![](http://images.iterate.site/blog/image/180817/fbg48J6i5A.png?imageslim){ width=55% }
...
![](http://images.iterate.site/blog/image/180817/labIEE4C4a.png?imageslim){ width=55% }
...
![](http://images.iterate.site/blog/image/180817/87gCAHdH3k.png?imageslim){ width=55% }
